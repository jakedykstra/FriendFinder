// stored data of friends survey answers
friends = [{
        "name": "Ahmed",
        "photo": "https://media.licdn.com/mpr/mpr/shrinknp_400_400/p/6/005/064/1bd/3435aa3.jpg",
        "scores": [
            5,
            1,
            4,
            4,
            5,
            1,
            2,
            5,
            4,
            1
        ]
    }], [{

    }]



    function friendMatch(friend){
        const reducer = (accumulator, currentValue) => accumulator + currentValue;

        for (i=0; i<friends.length; i++) {
            var overallScore = [];
            let score = friends[i]["scores"];
            score.reduce(reducer);
            overallScore.push(score);
        };

        let friendIndex = findIndex(friend)
        let friendScore = overallScore[friendIndex];
        for (i=0; i<overallScore.length; i++) {
            if (i == friendIndex){
                continue;
            } else if (!closest) {
                let closest = friendScore - overallScore[i]; 
                let closestIndex = i;
            } else {
                let lowCompare = friendScore - overallScore[i];
                if (lowCompare < closest) {
                    closet = lowCompare;
                    closestIndex = i;
                }
            }
                let closestMatch = friends[closestIndex];
                return jsonForModal(closestMatch);
        }
    }

    function jsonForModal(closestMatch){
    let similarFriend = {
        friendName: closestMatch.name,
        friendPicture: closestMatch.photo
    }
    return similarFriend;
    }


    // friendScore.map((val) => {
            //     let reducer = {};
            //     let name = Object.keys(val);
            //     let overall = Object.values(val).reduce(reducer);
            //     reducer[name] = overall;
            // });

    //     for (i=0; i<compareArr.length; i++) {
    //         compareArr[i][scores]
    //         for(j=0; j<compareArr[i][scores].length; i++){}
    //     }

    //     function compare(arr1,arr2){
    //         const finalarray =[];
    //       }
    
// 6. Determine the user's most compatible friend using the following as a guide:

// * Convert each user's results into a simple array of numbers (ex: `[5, 1, 4, 4, 5, 1, 2, 5, 4, 1]`).
// * With that done, compare the difference between current user's scores against those from other users, question by question. Add up the differences to calculate the `totalDifference`.
//   * Example: 
//     * User 1: `[5, 1, 4, 4, 5, 1, 2, 5, 4, 1]`
//     * User 2: `[3, 2, 6, 4, 5, 1, 2, 5, 4, 1]`
//     * Total Difference: **2 + 1 + 2 =** **_5_**
// * Remember to use the absolute value of the differences. Put another way: no negative solutions! Your app should calculate both `5-3` and `3-5` as `2`, and so on. 
// * The closest match will be the user with the least amount of difference.

// 7. Once you've found the current user's most compatible friend, display the result as a modal pop-up.
// * The modal should display both the name and picture of the closest match.